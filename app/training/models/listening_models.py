...
class ListeningExercise(BaseModel):
    ...
    difficulty_level: Mapped[DifficultyLevel] = mapped_column(nullable=False, comment="难度等级")

    # 练习内容
    audio_file_id: Mapped[int | None] = mapped_column(
        ForeignKey("listening_audio_files.id"), nullable=True, comment="音频文件ID（需求22可选）"
    )
    transcript: Mapped[str | None] = mapped_column(Text, nullable=True, comment="听力原文")

    # 题目信息
    questions_data: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False, comment="题目数据")
    ...
    def __repr__(self) -> str:
        return (
            f"<ListeningExercise(id={self.id}, title='{self.title}', type='{self.exercise_type}')>"
        )
...
class ListeningAudioFile(BaseModel):
    ...
    file_url: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="文件URL")
    ...
    sample_rate: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="采样率")
...
class ListeningSession(BaseModel):
    ...
    # 会话信息
    session_name: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="会话名称")
    ...
    total_questions: Mapped[int] = mapped_column(Integer, nullable=False, comment="总题目数")
...
class ListeningResult(BaseModel):
    ...
    unanswered: Mapped[int] = mapped_column(Integer, default=0, nullable=False, comment="未答题数")
    total_questions: Mapped[int] = mapped_column(Integer, nullable=False, comment="总题目数")
    ...
    total_time_seconds: Mapped[int] = mapped_column(Integer, nullable=False, comment="总用时（秒）")
    ...
    # 关联关系
    session: Mapped["ListeningSession"] = relationship("ListeningSession", back_populates="result")
    exercise: Mapped["ListeningExercise"] = relationship(
...
class ListeningSettings(BaseModel):
    ...
    exercise_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("listening_exercises.id"), nullable=True, index=True, comment="练习ID（可选）"
    )
...
class DictationExercise(BaseModel):
    ...
    exercise_id: Mapped[int] = mapped_column(Integer, ForeignKey("listening_exercises.id"), nullable=False)
    user_answers: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)
...
class SpeakingPractice(BaseModel):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    exercise_id: Mapped[int] = mapped_column(Integer, ForeignKey("listening_exercises.id"), nullable=False)
    audio_file_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("listening_audio_files.id"), nullable=True)
...
class PronunciationPractice(BaseModel):
    ...
    phoneme_scores: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)
