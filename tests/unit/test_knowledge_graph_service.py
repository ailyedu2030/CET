...
class TestKnowledgeGraphService:
    ...
    async def test_build_knowledge_graph_empty_library(self, knowledge_graph_service):
        """测试空知识库的图谱构建."""
        # 模拟空知识点列表
        with patch.object(knowledge_graph_service, '_get_knowledge_points', return_value=[]):
    ...
    async def test_build_knowledge_graph_with_data(self, knowledge_graph_service):
        ...
        with patch.object(knowledge_graph_service, '_get_knowledge_points', return_value=mock_knowledge_points):
            with patch.object(knowledge_graph_service, '_calculate_semantic_similarity', return_value=0.8):
                result = await knowledge_graph_service.build_knowledge_graph(library_id=1)
    ...
    async def test_get_knowledge_prerequisites(self, knowledge_graph_service):
        ...
        with patch.object(knowledge_graph_service, '_find_prerequisites', return_value=mock_prerequisites):
            result = await knowledge_graph_service.get_knowledge_prerequisites(knowledge_point_id)
    ...
    async def test_get_learning_path(self, knowledge_graph_service):
        ...
        with patch.object(knowledge_graph_service, '_find_shortest_path', return_value=mock_path):
            result = await knowledge_graph_service.get_learning_path(start_point_id, end_point_id)
    ...
    async def test_recommend_knowledge_points(self, knowledge_graph_service):
        ...
        mock_recommendations = [
            {"id": i, "title": f"推荐知识点{i}", "relevance_score": 0.9 - i*0.1, "difficulty": i%3+1}
            for i in range(1, limit+1)
        ]
        
        with patch.object(knowledge_graph_service, '_generate_recommendations', return_value=mock_recommendations):
            result = await knowledge_graph_service.recommend_knowledge_points(user_id, limit)
    ...
    def test_calculate_semantic_similarity(self, knowledge_graph_service):
        """测试语义相似度计算."""
        # 测试相同文本
        similarity = knowledge_graph_service._calculate_semantic_similarity("测试文本", "测试文本")
        assert similarity == 1.0
        
        # 测试不同文本
        similarity = knowledge_graph_service._calculate_semantic_similarity("完全不同", "另一个文本")
    ...
    async def test_error_handling(self, knowledge_graph_service):
        """测试错误处理."""
        # 测试无效library_id
        with patch.object(knowledge_graph_service, '_get_knowledge_points', side_effect=Exception("Database error")):
    ...
    def test_performance_metrics(self, knowledge_graph_service):
        ...
        # 验证权重配置合理性
        total_weight = (config['difficulty_weight'] + 
