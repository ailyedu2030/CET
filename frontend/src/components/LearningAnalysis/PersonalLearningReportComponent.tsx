...
const PersonalLearningReportComponent: React.FC<PersonalLearningReportComponentProps> = ({
  ...
  const getMasteryLabel = useCallback((level: number) => {
    const thresholds = Object.keys(MASTERY_LEVELS).map(Number).sort((a, b) => b - a)
  ...
  if (isLoading) {
    return (
      <Card withBorder padding="xl" style={{ position: 'relative', minHeight: UI_CONSTANTS.LOADING_MIN_HEIGHT }}>
    ...
    return (
      <Card withBorder padding="xl">
        <Stack align="center" gap="md">
          ...
          <Text size="lg" c="dimmed">暂无学情分析报告</Text>
    ...
    <Stack gap="md">
          ...
          <Group>
            ...
            <div>
              ...
              <Text size="sm" c="dimmed">
                ...
                分析周期：{new Date(report.report_period.start_date).toLocaleDateString()} - {new Date(report.report_period.end_date).toLocaleDateString()}
              </Text>
            </div>
          </Group>
      ...
      <Tabs value={activeTab} onChange={(value) => setActiveTab(value || 'overview')}>
          ...
          <Grid>
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder padding="md" h="100%">
                <Title order={4} mb="md">综合评分</Title>
                <Center>
                  <RingProgress
                    size={200}
                    thickness={20}
                    sections={[
                      { value: report.ability_assessment.overall_score, color: 'blue' },
                    ]}
                    label={
                      <Center>
            ...
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder padding="md" h="100%">
                <Title order={4} mb="md">关键指标</Title>
                <Stack gap="sm">
                  ...
                  <Group justify="space-between">
                    <Text>评估置信度</Text>
                    <Badge color="purple">{Math.round(report.ability_assessment.confidence_level * 100)}%</Badge>
                  </Group>
                </Stack>
              </Card>
            </Grid.Col>
        ...
        <Tabs.Panel value="ability" pt="md">
          <Card withBorder padding="md">
            <Title order={4} mb="md">五维能力评估</Title>
        ...
        <Tabs.Panel value="knowledge" pt="md">
          <Grid>
            <Grid.Col span={12}>
              <Card withBorder padding="md">
                <Title order={4} mb="md">知识点掌握热力图</Title>
            ...
            <Grid.Col span={12}>
              <Card withBorder padding="md">
                <Title order={4} mb="md">知识点详细分析</Title>
                        ...
                        <Paper key={point.knowledge_point_id} p="sm" withBorder>
                          <Group justify="space-between">
                            <div>
                              ...
                              <Text size="sm" c="dimmed">{point.category}</Text>
                            </div>
                            <Group>
                          ...
                          <Progress value={point.mastery_level} size="xs" mt="xs" color={mastery.color} />
                          <Group mt="xs" gap="xs">
                            <Text size="xs" c="dimmed">练习次数: {point.practice_count}</Text>
                            <Text size="xs" c="dimmed">错误次数: {point.error_count}</Text>
                            <Text size="xs" c="dimmed">预计掌握: {point.estimated_time_to_master}小时</Text>
                          </Group>
                        </Paper>
                      )
                    })}
        ...
        <Tabs.Panel value="progress" pt="md">
          <Card withBorder padding="md">
            <Title order={4} mb="md">学习进步趋势预测</Title>
            <div style={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Grid mt="md">
              <Grid.Col span={6}>
                <Paper p="md" withBorder>
                  <Text size="sm" c="dimmed" mb="xs">趋势方向</Text>
                  <Group>
                    <ThemeIcon
                      color={report.progress_prediction.trend_direction === 'improving' ? 'green' :
                             report.progress_prediction.trend_direction === 'declining' ? 'red' : 'yellow'}
                      variant="light"
                    >
                      <IconTrendingUp size={16} />
                    </ThemeIcon>
                    <Text fw={500}>
                      {report.progress_prediction.trend_direction === 'improving' ? '上升' :
                       report.progress_prediction.trend_direction === 'declining' ? '下降' : '稳定'}
                    </Text>
                  </Group>
                </Paper>
              </Grid.Col>
              <Grid.Col span={6}>
                <Paper p="md" withBorder>
                  <Text size="sm" c="dimmed" mb="xs">预期提升率</Text>
        ...
        <Tabs.Panel value="recommendations" pt="md">
          <Grid>
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder padding="md" h="100%">
                <Title order={4} mb="md">优先改进领域</Title>
                <Stack gap="sm">
                  {report.recommendations.priority_areas.map((area, index) => (
                    <Paper key={index} p="sm" withBorder>
                      <Group justify="space-between" mb="xs">
                        ...
                        <Badge
                          color={area.importance === 'high' ? 'red' :
                                 area.importance === 'medium' ? 'orange' : 'blue'}
                          variant="light"
                        >
                          {area.importance === 'high' ? '高优先级' :
                           area.importance === 'medium' ? '中优先级' : '低优先级'}
                        </Badge>
                      </Group>
            ...
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder padding="md" h="100%">
                <Title order={4} mb="md">学习计划</Title>
                <Timeline active={0} bulletSize={UI_CONSTANTS.TIMELINE_BULLET_SIZE} lineWidth={UI_CONSTANTS.TIMELINE_LINE_WIDTH}>
          ...
          <Stack gap="md">
            ...
            <Grid>
              <Grid.Col span={{ base: 12, md: 8 }}>
                <Card withBorder padding="md">
                  <Title order={4} mb="md">风险因素分析</Title>
                  <Stack gap="sm">
                    {report.risk_warning.risk_factors.map((factor, index) => (
                      <Paper key={index} p="sm" withBorder>
                        <Group justify="space-between" mb="xs">
                          ...
                          <Badge
                            color={factor.severity >= 80 ? 'red' :
                                   factor.severity >= 60 ? 'orange' :
                                   factor.severity >= 40 ? 'yellow' : 'blue'}
                            variant="light"
                          >
                            严重程度: {factor.severity}%
                          </Badge>
                        </Group>
                        <Text size="sm" mb="xs">{factor.description}</Text>
                        <Text size="sm" c="blue">
              ...
              <Grid.Col span={{ base: 12, md: 4 }}>
                <Card withBorder padding="md">
                  <Title order={4} mb="md">干预建议</Title>
                  <List size="sm" spacing="xs">
                    {report.risk_warning.intervention_recommendations.map((recommendation, index) => (
