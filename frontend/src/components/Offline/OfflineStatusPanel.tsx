...
export function OfflineStatusPanel(): JSX.Element {
  ...
  const { isSync, lastSyncTime, pendingItems, errors, manualSync } = useSyncStatus()
        ...
        <Stack gap="md">
          ...
          <Card withBorder>
            <Stack gap="md">
              ...
              {pendingItems > 0 && (
                <Progress value={isSync ? 50 : 0} color="blue" size="sm" animated={isSync} />
              )}
          ...
          <Card withBorder>
            <Stack gap="md">
              <Group justify="space-between">
                ...
                <ActionIcon size="sm" variant="outline" onClick={refreshStats}>
                  <IconRefresh size={14} />
                </ActionIcon>
              </Group>
          ...
          {hasConflicts && (
            <Alert color="orange" icon={<IconAlertTriangle size={16} />}>
              <Group justify="space-between">
                <Text size="sm">发现 {conflicts.length} 个数据冲突，需要手动解决</Text>
                <Button size="xs" variant="outline" onClick={() => setConflictModalOpened(true)}>
                  解决
                </Button>
              </Group>
            </Alert>
