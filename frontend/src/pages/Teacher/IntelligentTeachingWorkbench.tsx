...
export function IntelligentTeachingWorkbench(): JSX.Element {
  ...
  const { data: learningAnalysis, isLoading: analysisLoading } = useQuery({
    ...
    queryFn: () =>
      learningAnalysisApi.comprehensiveAnalysis({
  ...
  useQuery({
    ...
    queryFn: () =>
      effectTrackingApi.getProgressAnalysis({
        period_days: 30,
      }),
  ...
  return (
    <Container size="xl" py="md">
        ...
        <Group>
          ...
          <Menu>
            ...
            <Menu.Dropdown>
              ...
              <Menu.Item leftSection={<IconUsers size={14} />}>协作设置</Menu.Item>
            </Menu.Dropdown>
          </Menu>
        </Group>
      </Group>
      ...
      {/* 系统状态指示器 */}
      {systemHealth && (
        <Alert icon={<IconCheck size={16} />} color="green" mb="md" variant="light">
          系统运行正常 - 所有AI服务可用
        </Alert>
      )}
      ...
      <Tabs value={activeTab} onChange={value => setActiveTab(value || 'overview')}>
          ...
          <Grid>
              ...
              <Card withBorder>
                <Group justify="space-between">
                  <div>
                    ...
                    <Text size="xl" fw={700}>
                      {Math.round(
                        (learningAnalysis?.analysis_metadata?.data_quality_score || 0) * 100
                      )}
                      %
                    </Text>
                  </div>
              ...
              <Card withBorder>
                <Group justify="space-between">
                  <div>
                    ...
                    <Text size="xl" fw={700}>
                      {Math.round(
                        (learningAnalysis?.analysis_metadata?.data_quality_score || 0) * 85
                      )}
                      %
                    </Text>
                  </div>
            ...
            <Grid.Col span={12}>
              <Card withBorder>
                ...
                {learningAnalysis?.risk_assessment?.risk_students &&
                learningAnalysis.risk_assessment.risk_students.length > 0 ? (
                  <Stack gap="sm">
                    {learningAnalysis.risk_assessment.risk_students
                      .slice(0, 3)
                      .map((riskStudent, index) => (
                        <Alert
                          key={index}
                          icon={<IconAlertTriangle size={16} />}
                          color={
                            riskStudent.risk_level === 'high'
                              ? 'red'
                              : riskStudent.risk_level === 'medium'
                                ? 'orange'
                                : 'yellow'
                          }
                          variant="light"
                        >
                          <Group justify="space-between">
                            <div>
                              <Text fw={500}>
                                学生ID: {riskStudent.student_id} - 风险等级:{' '}
                                {riskStudent.risk_level}
                              </Text>
                              <Text size="sm" c="dimmed">
            ...
            {learningAnalysis ? (
              <Stack gap="md">
                ...
                <div>
                  ...
                  <Stack gap="xs">
                    {learningAnalysis.risk_assessment?.warning_indicators?.map(
                      (indicator, index) => (
        ...
        <Tabs.Panel value="adjustments" pt="md">
          <Card withBorder>
            ...
            <Text c="dimmed">基于学情分析结果，系统将为您生成个性化的教学调整建议</Text>
          </Card>
        </Tabs.Panel>
        ...
        <Tabs.Panel value="tracking" pt="md">
          <Card withBorder>
            ...
            <Text c="dimmed">跟踪教学调整的实施效果，提供数据驱动的改进建议</Text>
          </Card>
        </Tabs.Panel>
        ...
        <Tabs.Panel value="collaboration" pt="md">
          <Card withBorder>
            ...
            <Text c="dimmed">与其他教师分享成功的教学调整经验，协同优化教学方案</Text>
          </Card>
        </Tabs.Panel>
      </Tabs>
    </Container>
