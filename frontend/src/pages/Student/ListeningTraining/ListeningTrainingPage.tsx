...
export const ListeningTrainingPage: FC = () => {
  ...
  const [selectedExerciseType, setSelectedExerciseType] =
    useState<ListeningExerciseType>('short_conversation')
  ...
  const { data: weakPoints } = useQuery({
    ...
    queryFn: () =>
      currentSession?.exercise_id
        ? listeningApi.getWeakPoints(currentSession.exercise_id)
  ...
  const { data: tips } = useQuery({
    ...
    queryFn: () =>
      currentSession?.exercise_id
        ? listeningApi.getListeningTips(currentSession.exercise_id)
        : Promise.resolve(null),
    enabled: !!currentSession?.exercise_id,
  })
  ...
  const renderExerciseTypeCard = useCallback(
        ...
        <Tabs.Panel value="practice" pt="md">
          ...
          <Card withBorder mb="lg">
            ...
            <Grid>
              {useMemo(
                () =>
                  (Object.keys(EXERCISE_TYPE_CONFIG) as ListeningExerciseType[]).map(
          ...
          <Card withBorder>
            <Group justify="space-between" mb="md">
              ...
              <Badge color="blue">{EXERCISE_TYPE_CONFIG[selectedExerciseType].name}</Badge>
            </Group>
                    ...
                    <Card key={index} withBorder padding="md">
                      <Group justify="space-between" mb="xs">
                        ...
                        <RingProgress
                          ...
                          sections={[
                            {
                              value: phonetic.accuracy_rate * 100,
                              color:
                                phonetic.accuracy_rate > 0.8
                                  ? 'green'
                                  : phonetic.accuracy_rate > 0.6
                                    ? 'yellow'
                                    : 'red',
                            },
                          ]}
          ...
          <Stack gap="md">
            ...
            <Card withBorder>
              <Group justify="space-between" mb="md">
                ...
                <Badge
                  color={
                    trainingResult.score >= 80
                      ? 'green'
                      : trainingResult.score >= 60
                        ? 'yellow'
                        : 'red'
                  }
                  size="lg"
              ...
              <Grid>
                <Grid.Col span={6}>
                  ...
                  <Text fw={500}>
                    {trainingResult.correct_count} / {trainingResult.total_questions}
                  </Text>
                </Grid.Col>
                <Grid.Col span={6}>
                  <Text size="sm" c="dimmed">
                  ...
                  <Text fw={500}>
                    {Math.round(
                      (trainingResult.correct_count / trainingResult.total_questions) * 100
                    )}
                    %
                  </Text>
                </Grid.Col>
            ...
            <Card withBorder>
              ...
              {trainingResult.improvement_suggestions?.length > 0 ? (
                <Stack gap="xs">
                  {trainingResult.improvement_suggestions.map(
